...
export function ResultsCard({ analysis: a, summary, extraWarnings = [] }: ResultsCardProps) {
  ...
  const g = a.gleason
  const warnings = [...a.warnings.map((w) => t(`prostate.warnings.${w.key}`, w.params ?? {})), ...extraWarnings]

  const tiles: { label: string; value: string; sub?: string }[] = [
    {
      label: t('prostate.results.volume'),
      value: `${n(a.volumePct)}%`,
      sub: a.tumorGrams !== null ? `≈ ${n(a.tumorGrams, 2)} g` : t('prostate.results.involved', { n: a.involvedCells, total: a.totalCells }),
    },
    {
      label: 'Gleason',
    ...
    { label: t('prostate.results.g5'), value: a.shares ? `${n(a.shares.p5)}%` : '—' },
    {
      ...
      sub: a.margins.foci.length
        ? `${a.margins.foci.length} ${t('prostate.results.foci')}${a.margins.extent ? ` · ${t(`prostate.results.${a.margins.extent}`)}` : ''}`
        : undefined,
    },
    {
  ...
  return (
    <div className="rounded-lg border border-line bg-elevated shadow-card">
      <SectionHeader title={t('prostate.results.title')} hint={t('prostate.results.hint')} />
      <div className="space-y-5 px-5 py-5">
        <dl className="grid grid-cols-2 gap-px overflow-hidden rounded-md border border-line bg-line sm:grid-cols-3">
          {tiles.map((tile) => (
            <div key={tile.label} className="bg-surface px-3 py-2.5">
              <dt className="text-[0.6875rem] tracking-wider text-ink-faint uppercase">{tile.label}</dt>
              <dd className="tabular mt-1 text-lg font-medium text-accent-ink">{tile.value}</dd>
        ...
        <div>
          <div className="flex items-center justify-between gap-3">
            ...
            <Button type="button" size="sm" variant="secondary" onClick={() => void copy()} disabled={!summary}>
